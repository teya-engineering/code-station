...
struct PullRequestTests {
    ...
    @Test func readsTheLinkGhPrintsWhenItOpensOne() {
        let found = PullRequestScanner.opened(
            ...
            https://github.com/teya/conductor/pull/123
            """)
        #expect(found == PullRequest(number: 123, url: "https://github.com/teya/conductor/pull/123"))
    }
    ...
    @Test func ignoresCommandsThatOnlyLookAtOne() {
        #expect(PullRequestScanner.opened(command: #"{"command":"gh pr view 5 --json url"}"#,
                                          output: "https://github.com/teya/conductor/pull/5") == nil)
        #expect(PullRequestScanner.opened(command: #"{"command":"git push -u origin HEAD"}"#,
                                          output: "remote: https://github.com/teya/conductor/pull/new/session-4") == nil)
    }

    // Trailing punctuation is part of the sentence, not of the link.
    @Test func leavesPunctuationOutOfTheLink() {
        let found = PullRequestScanner.scan("Opened https://github.com/teya/conductor/pull/12.")
        #expect(found?.url == "https://github.com/teya/conductor/pull/12")
    }
    ...
    @Test func picksTheLastPullRequestTheSessionOpened() {
        ...
        session.messages = [
            message(command: "gh pr create --fill", result: "https://github.com/teya/conductor/pull/1"),
            message(command: "gh pr view 1", result: "https://github.com/teya/conductor/pull/1"),
            message(command: "gh pr create --fill", result: "https://github.com/teya/conductor/pull/2")
        ]
